...
async def upload_documents(
    ...
    callback_url: Optional[str] = Form(
        None,
        description="任务完成后的回调地址（HTTP/HTTPS）",
    ),
    ...
    try:
        ...
        normalized_callback_url: Optional[str] = None
        if callback_url is not None:
            stripped_callback = callback_url.strip()
            if stripped_callback:
                parsed = urlparse(stripped_callback)
                if parsed.scheme not in {"http", "https"} or not parsed.netloc:
                    raise HTTPException(
                        status_code=400,
                        detail="callback_url 必须是有效的 HTTP/HTTPS 地址",
                    )
                normalized_callback_url = stripped_callback
        ...
        for file in files:
            ...
            if hasattr(file, "size") and file.size > 100 * 1024 * 1024:
                raise HTTPException(
                    status_code=400, detail=f"文件 {file.filename} 过大，最大支持100MB"
                )
...
async def list_documents():
    ...
    try:
        ...
        if not data_dir.exists():
            return DocumentListResponse(documents=[], total_count=0, total_size=0)
        ...
        return DocumentListResponse(
            documents=documents, total_count=len(documents), total_size=total_size
        )
...
async def rebuild_index(request: IndexRebuildRequest, tenant_id: str = Depends(get_tenant_id)):
    ...
    try:
        ...
        if not all_files:
            raise HTTPException(status_code=400, detail="数据目录中没有文件，无法重建索引")

        # 如果不是强制重建，检查索引是否已存在
        if not request.force and knowledge_service.has_index(tenant_id=tenant_id):
            return {"message": "索引已存在，使用 force=true 强制重建", "index_exists": True}

        # 异步重建索引
        documents = await knowledge_service.process_documents_async(all_files)
        await knowledge_service.build_index_async(
            documents, reset_existing=True, tenant_id=tenant_id
        )
...
async def update_config(config: ConfigUpdate):
    ...
    try:
        ...
        if config.chunk_size is not None:
            ...
            Settings.node_parser = SentenceSplitter(chunk_size=config.chunk_size, chunk_overlap=20)
            updated_config["chunk_size"] = config.chunk_size

        return {"message": "配置更新成功", "updated_config": updated_config}
